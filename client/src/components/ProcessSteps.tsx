      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, index) => (
            <Card key={index} className="relative overflow-hidden hover-elevate cursor-pointer group" data-testid={`card-step-${step.step}`}>
              <CardContent className="p-6 text-center">
                <div className="absolute top-4 right-4 text-4xl font-bold text-accent/20 group-hover:text-accent/40 transition-colors">
                  {step.step}
                </div>
                <div className="w-16 h-16 bg-chart-3 rounded-2xl flex items-center justify-center mx-auto mb-4">
