...
## Related: null vs undefined in PATCH zod schemas
These edit forms send `field.trim() || null` for empty optional fields, so the body carries `null` (not `undefined`). A backend `z.string().optional()` accepts undefined but REJECTS null → 400 "Invalid data" whenever a nullable field (phone/company/email) is left blank. Make nullable columns `z.string().optional().nullable()` (and for email, `z.union([z.string().email(), z.literal(""), z.null()]).optional()`). Symptom: "Failed to update customer" toast + 400 in console, easy to misread as an auth/route problem.
